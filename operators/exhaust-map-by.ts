import {
  ...
  export function exhaustMapBy<T, O extends ObservableInput<any>>(property: string, project: (value: T, index: number) => O): OperatorFunction<T, ObservedValueOf<O>> {
    return (source: Observable<T>) => source.lift(new ExhaustMapByOperator(property, project));
  }
  /* tslint:enable:max-line-length */
  
  class ExhaustMapByOperator<T, R> implements Operator<T, R> {
  ...
  class SwitchMapBySubscriber<T, R> extends OuterSubscriber<T, R> {
    ...
    private _innerSub(result: ObservableInput<R>, value: T, index: number) {
      ...
      if(this.subscriptionMap.has(id)){
        console.warn('exhaustMapBy :: skipping :: ',id);
        return;
      }
  
      console.warn('exhaustMapBy :: serving :: ',id);
    ...
    protected _complete(): void {
      if(!this.subscriptionMap.size){
        console.log('exhaustMapBy :: complete');
        super._complete();
      }
    }
  
    protected _unsubscribe() {
      console.log('exhaustMapBy :: unsubscribe');
      this.unsubscribeAll();
      this.unsubscribe();
    }
  
    notifyComplete(innerSub: Subscription): void {
      console.log('exhaustMapBy :: inner-subscription :: completed');
      const destination = this.destination as Subscription;
